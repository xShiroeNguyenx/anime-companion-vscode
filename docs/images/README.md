...
| 14 | `14-outfit-expression-popups.png` | **Outfits & Expressions (v0.5.5).** Two shots side by side (or one composite): left, right-click › Appearance › **Outfit** open on a model that ships outfit `.exp3.json` files, showing the **Default** row + outfit names with one marked active; right, Appearance › **Expression** open on **Mao** listing `exp_01…exp_08`, with the model visibly wearing the selected face. Not referenced by the READMEs yet — add the image reference under the "👗 Outfits & 😊 Expressions" section once captured. `scripts/outfit-harness.js` writes a similar shot to `.harness/panels.png` if you'd rather start from that. |
