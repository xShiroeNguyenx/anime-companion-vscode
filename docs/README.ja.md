...
> 🆕 **v0.5.1 の新着**:
> - **🌸 専用ウィンドウの Markdown WYSIWYG エディター** — 任意の `.md` ファイルを、かわいいフルサイズのエディターで開いて見たまま編集できます（変更はファイルに直接書き込み）。エディターのタイトルバーの **🌸 お花**ボタン（およびステータスバーで点滅する **🌸**）で開き、レンダリングされた文書をリッチテキストエディターのように（CKEditor 風に）編集できます。**🌗 ダーク / ライト**切り替えと Anime Companion 風のスタイリング付き。**設計上安全** — 実際に編集したときだけファイルに書き込むので、プレビューだけでは書式が崩れません。
>
> **v0.5.0**（🖼️ 背景画像の操作パネル）、**v0.4.3**（Claude アカウント切替）、**v0.4.0**（Agent Accounts、💬 Pet Quick Chat）を土台にしています。
