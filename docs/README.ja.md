...
> 🆕 **v0.5.2 の新着**:
> - **🎨 Markdown エディターのテーマカラーを変更** — ヘッダーのカラースウォッチで、ピンクのクローム全体（ヘッダー、ボタン、枠線、リンク、スクロールバー）を好きな色に変えられます。**↺** ボタンでデフォルトの桜ピンクに戻せます。選択はファイル・ウィンドウをまたいで記憶されます。背景色は引き続き **🌗 ダーク / ライト**モードに従います。
> - **細いスクロールバー** — テーマカラーに合わせた細いバーになり（対応環境では自動で隠れます）、live/Markdown・WYSIWYG ペインの太い既定バーを置き換えます。
>
> **v0.5.1**（🌸 Markdown WYSIWYG エディター）、**v0.5.0**（🖼️ 背景画像の操作パネル）、**v0.4.3**（Claude アカウント切替）、**v0.4.0**（Agent Accounts、💬 Pet Quick Chat）を土台にしています。
