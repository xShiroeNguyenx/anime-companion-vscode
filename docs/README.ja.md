...
> 🆕 **v0.5.4 の新着**:
> - **🔄 手でモデルを回す** — **Alt + 左ドラッグ**で、ドラッグした方向へ相棒の頭と体を向けられます（Live2D のフォーカスコントローラーによる、範囲制限つきの擬似 3D の「見る」動作。離すと元の姿勢へ戻ります）。
> - **👀 カーソルを自動で目で追う** — 右クリックメニューの新しいトグル（**外観 › カーソル追従**）で、カーソルが相棒の上にある間、頭と目が自動で追いかけます（設定は記憶されます）。カーソルを速く左右に振りすぎると、相棒が「目が回る」と言ってゆっくりにとお願いします。
>
> **v0.5.3**（🔗 URL からの背景画像）、**v0.5.1**（🌸 Markdown WYSIWYG エディター）、**v0.5.0**（🖼️ 背景画像の操作パネル）、**v0.4.3**（Claude アカウント切替）、**v0.4.0**（Agent Accounts、💬 Pet Quick Chat）を土台にしています。
