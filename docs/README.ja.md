...
> - **🖐️ カーソルを置いた場所が反応します** — 体の一部にカーソルを止めると、その部分が反応します。頭では照れ、胸やスカートでは慌てて目をそらし、手では手を振ります。部位名と操作を示す小さなキャプション（「頭 · 長押しで表情とモーション」）が出て、その部分が一瞬縁取られ、カーソルも部位ごとに変わります — 髪の上ではくし、体の上ではドレス、触ってほしくない場所では斜線入りのハートやリボン、手の上では開いた手 — どれもハートカーソルと同じピンクの矢印ファミリーです。**止めた**ときだけ反応し、各部位はそのあと数秒休みます。`animeCompanion.hoverReactions.enabled: false` でオフ。
