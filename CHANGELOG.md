...
- **A caption names the part and the gesture it takes** — "Head · hold to see expressions & motions", "Body · hold to change outfits" — so the press-and-hold gestures are discoverable by pointing at them, not only by waiting for an idle hint. The refused parts answer in the character's own voice instead. The caption sits **beside** the character rather than over it, on whichever side has room (under the model when neither side does), on an opaque dark pill: over hair and fabric the text could not be read, and the parts most worth labelling are exactly the ones with the busiest background.
